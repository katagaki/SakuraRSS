...
extension InstagramProfileFetcher {
    ...
    func fetchPostComments(shortcode: String, limit: Int) async -> [ParsedInstagramComment] {
        ...
        do {
            ...
            log("InstagramProfileFetcher", "Comments page network error: \(error)")
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            log("InstagramProfileFetcher", "Comments page bad status: \((response as? HTTPURLResponse)?.statusCode ?? -1)")
            return []
        }
