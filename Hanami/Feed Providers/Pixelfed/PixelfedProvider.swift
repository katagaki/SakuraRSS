...
public final class PixelfedProvider {

    public nonisolated static let knownHosts: Set<String> = [
        "pixelfed.social",
        "pixelfed.tokyo",
        "pixelfed.art"
    ]

    public nonisolated static func isPixelfedHost(_ host: String?) -> Bool {
        matchesHost(host)
    }

    /// Profile pages live at the bare `<host>/<username>` path on Pixelfed,
    /// not the `@username` form Mastodon uses.
    public nonisolated static func profileURL(host: String, username: String) -> URL? {
        URL(string: "https://\(host)/\(username)")
    }
    public func fetchProfile(host: String, username: String) async -> PixelfedProfileFetchResult {
