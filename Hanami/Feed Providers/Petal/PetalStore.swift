...
public nonisolated final class PetalStore: @unchecked Sendable {

    public static let shared = PetalStore()
    ...
    public func save(_ recipe: PetalRecipe) throws {
        var copy = recipe
    ...
    public func recipe(id: UUID) -> PetalRecipe? {
    ...
    public func iconURL(for id: UUID) -> URL {
        iconDirectoryURL.appendingPathComponent("\(id.uuidString).png")
    }

    public func saveIcon(_ data: Data, for id: UUID) throws {
        try data.write(to: iconURL(for: id), options: .atomic)
    }

    public func iconData(for id: UUID) -> Data? {
