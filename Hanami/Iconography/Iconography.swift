...
public actor Iconography {
    ...
    public static let shared = Iconography()

    /// Dedicated short-timeout URLSession for icon fetches.
    public nonisolated static let urlSession: URLSession = {
    ...
    public func clearFailedLookups(for entries: [(domain: String, siteURL: String?)]) {
        for entry in entries {
            let cacheKey = Self.cacheKey(domain: entry.domain, siteURL: entry.siteURL)
            forgetFailedLookup(cacheKey)
        }
    ...
    public nonisolated static func cacheKey(domain: String, siteURL: String?) -> String {
    ...
    public func sanitizedFileName(_ key: String) -> String {
    ...
    public func metricsSidecarURL(for cacheKey: String) -> URL {
