...
public nonisolated enum FeedIconAlternateDomains: DomainDefaults {
    ...
    public static var exceptionDomains: Set<String> { Set(mappings.keys) }

    /// Returns the mapped domain for icon fetching, or the original domain if no mapping exists.
    public static func iconDomain(for feedDomain: String) -> String {
