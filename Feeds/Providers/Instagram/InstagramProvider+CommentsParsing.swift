...
extension InstagramProvider {
    ...
    static func parseCommentsHTML(_ html: String, shortcode: String) -> [ParsedInstagramComment] {
        ...
        guard let payload = applicationJSONScript(containing: key, in: html) else {
            log("InstagramProvider", "Comments JSON script not found in HTML")
            return []
        }

        guard let data = payload.data(using: .utf8),
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            log("InstagramProvider", "Comments JSON failed to deserialize")
            return []
        }

        guard let edges = findCommentEdges(in: json) else {
            log("InstagramProvider", "Comments edges not found in JSON tree")
            return []
        }
