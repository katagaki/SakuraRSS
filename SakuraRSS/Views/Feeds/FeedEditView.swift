...
struct FeedEditView: View {
    ...
    init(feed: Feed) {
        ...
        let initialURL: String = (feed.isXFeed || feed.isInstagramFeed || feed.isYouTubePlaylistFeed)
            ? feed.siteURL : feed.fetchURL
        _url = State(initialValue: initialURL)
        let existingIconURL = feed.customIconURL
        let initialIconInput: String = (existingIconURL == "photo" || existingIconURL == "none")
            ? "" : (existingIconURL ?? "")
        _iconURLInput = State(initialValue: initialIconInput)
        ...
        _openMode = State(initialValue: openModeRaw.flatMap(FeedOpenMode.init(rawValue:)) ?? .inAppViewer)
        let articleSourceRaw = UserDefaults.standard.string(forKey: "articleSource-\(feed.id)")
        _articleSource = State(initialValue: articleSourceRaw.flatMap(ArticleSource.init(rawValue:)) ?? .automatic)
    }
